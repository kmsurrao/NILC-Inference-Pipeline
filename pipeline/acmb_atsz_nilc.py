...
def get_all_acmb_atsz(Nsims, ellmax, scratch_path, verbose):
    ...
    def load_Clpq():
        ...
        '''
        data_spectra = np.zeros((Nsims, 3, 3, ellmax+1)) 
        for sim in range(Nsims):
            data_spectra[sim] = pickle.load(open(f'{scratch_path}/power_spectra/{sim}_data_spectra.p', 'rb'))
    ...
    pickle.dump(acmb_array, open(f'{scratch_path}/acmb_array.p', 'wb'))
    pickle.dump(atsz_array, open(f'{scratch_path}/atsz_array.p', 'wb'))
    if verbose:
        print(f'created {scratch_path}/acmb_array.p and {scratch_path}/atsz_array.p', flush=True)
...
def get_parameter_cov_matrix(Nsims, ellmax, scratch_path, verbose, nbins=100, smoothing_factor=0.065):
    acmb, atsz = get_all_acmb_atsz(Nsims, ellmax, scratch_path, verbose)
