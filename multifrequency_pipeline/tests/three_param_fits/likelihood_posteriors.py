...
def ClijA(A1, A2, A3, inp, Clij00, Clij01, Clij10, Clij11):
    '''
    ...
    A3: float, scaling parameter for power spectrum of third component
    ...
    Clij_with_A_00 = A1*Clij00[1] + A2*Clij00[2] + A3*Clij00[3]
    Clij_with_A_01 = A1*Clij01[1] + A2*Clij01[2] + A3*Clij01[3]
    Clij_with_A_10 = A1*Clij10[1] + A2*Clij10[2] + A3*Clij10[3]
    Clij_with_A_11 = A1*Clij11[1] + A2*Clij11[2] + A3*Clij11[3]
...
def acmb_atsz_numerical(inp, sim, Clij00_all_sims, Clij01_all_sims, Clij10_all_sims, Clij11_all_sims, PScov_sim_Inv):
    '''
    Maximize likelihood with respect to A1, A2, A3 for one sim using numerical minimization routine
    ...
    RETURNS
    -------
    best fit A1, A2, A3 (floats)
    '''
    all_res = []
    for start in [0.5, 1.0, 1.5]:
        start_array = [start, start, start] #a1_start, a2_start, a3_start
...
def acmb_atsz_analytic(inp, sim, Clij00_all_sims, Clij01_all_sims, Clij10_all_sims, Clij11_all_sims, PScov_sim_Inv):
    '''
    Maximize likelihood with respect to A1, A2, A3 for one sim analytically 
...
def get_MLE_arrays(inp, Clij00_all_sims, Clij01_all_sims, Clij10_all_sims, Clij11_all_sims, PScov_sim_Inv, use_analytic=True):
    ...
    a3_array: array of length Nsims containing best fit A3 for each simulation
    '''
    ...
    pool.close()
    param_array = np.asarray(param_array, dtype=np.float32) #shape (Nsims, 3 for A1 A2 A3)
    a1_array = param_array[:,0]
    a2_array = param_array[:,1]
    a3_array = param_array[:,2]
    

    final_cov = np.cov(np.array([a1_array, a2_array, a3_array]))
    ...
    print(f'A3 = {np.mean(a3_array)} +/- {np.sqrt(final_cov[2,2])}', flush=True)

    return a1_array, a2_array, a3_array
...
def Fisher_inversion(inp, Clij, PScov_sim_Inv):
    ...
    RETURNS
    -------
    a1_std, a2_std, a3_std: predicted standard deviations of A1, etc.
    ...
    print('A3 std dev: ', a3_std, flush=True)
...
def MCMC(inp, Clij00_all_sims, Clij01_all_sims, Clij10_all_sims, Clij11_all_sims, PScov_sim_Inv, sim=0):
    ...
    samples = sampler.get_chain() #dimensions (1000, nwalkers, Ncomps=3)
    ...
    a3_std = np.mean(np.array([np.std(samples[:,walker,2]) for walker in range(nwalkers)]))
...
def cov_of_MLE_analytic(Clij00_all_sims, Clij01_all_sims, Clij11_all_sims, PScov_sim_Inv):
    ...
    print('A3 std dev: ', np.sqrt(F_inv[2,2]), flush=True)
    return F_inv 
...
def get_all_acmb_atsz(inp, Clij):
    ...
    a1_array, a2_array, a3_array = get_MLE_arrays(inp, Clij00_all_sims, Clij01_all_sims, Clij10_all_sims, Clij11_all_sims, PScov_sim_Inv, use_analytic=True)
    print(flush=True)
    a1_array, a2_array, a3_array = get_MLE_arrays(inp, Clij00_all_sims, Clij01_all_sims, Clij10_all_sims, Clij11_all_sims, PScov_sim_Inv, use_analytic=False)
