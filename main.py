...
def one_sim(sim, inp, env, wigner):
    ...
    data_spectra = get_data_spectra(sim, inp.freqs, inp.Nscales, inp.tsz_amp, inp.ellmax, wigner, CC, T, N, wt_map_power_spectrum, inp.GN_FWHM_arcmin, inp.scratch_path, inp.verbose)
    if inp.verbose:
...
if __name__ == '__main__':
    ...
    with MPICommExecutor(MPI.COMM_WORLD, root=0) as executor:
        if executor is not None:
            ...
            iterable = [(sim, inp, my_env, wigner) for sim in range(inp.Nsims)]
