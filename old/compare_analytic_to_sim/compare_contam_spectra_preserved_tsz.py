...
wt_map_power_spectrum = get_wt_map_spectra(sim, inp.ellmax, inp.Nscales, inp.verbose, inp.scratch_path, comps=['tSZ'])
if inp.verbose:
...
W_tsz = get_comp_and_wt_map_cross_spectra(sim, inp.ellmax, inp.Nscales, tsz_map, inp.verbose, inp.scratch_path, ['tSZ'])[1]
W_cmb = get_comp_and_wt_map_cross_spectra(sim, inp.ellmax, inp.Nscales, cmb_map, inp.verbose, inp.scratch_path, ['tSZ'])[1]
if inp.verbose:
...
CC_nilc = calculate_all_cl_corrected(nfreqs, inp.ellmax, h, a, CC, M, W_cmb, W_cmb, wigner_zero_m, wigner_nonzero_m) #CMB propagation from our equation
T_nilc = calculate_all_cl_corrected(nfreqs, inp.ellmax, h, g, T, M, W_tsz, W_tsz, wigner_zero_m, wigner_nonzero_m) #tSZ propagation from our equation, should be unbiased
if inp.verbose:
