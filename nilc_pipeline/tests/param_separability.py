...
def get_scaled_maps_and_wts(sim, inp, env):
    ...
    for y in range(2*N_comps+1):
        ...
        CC, T, N1, N2, CMB_map, tSZ_map, noise1_map, noise2_map = generate_freq_maps(sim, inp, scaling=scaling, save=False)
...
def main():
    ...
    pool.close()
    Clpq = np.asarray(Clpq, dtype=np.float32) #shape (Nsims, N_preserved_comps=2, N_preserved_comps=2, N_comps=4, N_comps=4, ellmax+1)
    if inp.save_files:
