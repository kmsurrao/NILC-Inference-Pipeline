...
def calculate_all_cl_corrected(nfreqs, ellmax, h, g, cl, M, Wp, Wq, wigner_zero_m, wigner_nonzero_m, delta_ij=False):
    ...
    wigner_nonzero_m: 3D array of wigner3j symbols of form (l1 l2 l2 0 -m2 m2), index by wigner[l1][l2][m2]
    delta_ij: True if delta_{ij} is attached to term. False by default
    ...
    if not delta_ij:
        ...
        term3 = float(1/(4*np.pi))*np.einsum('a,b,laa,lbb,lac,lbd,nl,ml,na,mb,i,j,nia,mjb->l',2*l2+1,2*l3+1,wigner_zero_m,wigner_zero_m,wigner_nonzero_m,wigner_nonzero_m,h[:,:ellmax+1],h[:,:ellmax+1],h,h,g,g,Wp,Wq,optimize=True)
    else:
        ...
        term3 = float(1/(4*np.pi))*np.einsum('a,b,laa,lbb,lac,lbd,nl,ml,na,mb,i,i,nia,mib->l',2*l2+1,2*l3+1,wigner_zero_m,wigner_zero_m,wigner_nonzero_m,wigner_nonzero_m,h[:,:ellmax+1],h[:,:ellmax+1],h,h,g,g,Wp,Wq,optimize=True)
